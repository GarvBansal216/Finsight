...
export default function HeroSection() {
  // Generate consistent pixel pattern for financial dashboard
  const pixelColors = useMemo(() => {
    const pixels = [];
    const colors = ["bg-purple-500", "bg-yellow-400", "bg-blue-500", "bg-gray-800"];
    // Use a seeded pattern for consistency
    const pattern = [
      3, 0, 3, 3, 1, 3, 3, 2, 3, 3, 3, 0, 3, 3, 1, 3,
      3, 2, 3, 3, 3, 0, 3, 1, 3, 3, 2, 3, 3, 3, 0, 3,
      3, 1, 3, 3, 2, 3, 3, 3, 0, 3, 3, 1, 3, 2, 3, 3
    ];
    for (let i = 0; i < 48; i++) {
      pixels.push(colors[pattern[i] || 3]);
    }
    return pixels;
  }, []);
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid lg:grid-cols-2 gap-8 lg:gap-16 items-center">
          <div className="text-center lg:text-left">
            ...
            <p className="text-base sm:text-lg text-gray-300 mb-6 sm:mb-8 max-w-lg mx-auto lg:mx-0 leading-relaxed">
              FinSight Embraces The Era Of Intelligent Financial Computing, Enabling Swift And Effortless
              Document Processing With Just A Tap. No More Fumbling With Spreadsheets Or Struggling With Manual Data Entry.
            </p>
          </div>
          ...
          <div className="relative mt-8 lg:mt-0">
            ...
            <div className="relative bg-gray-900 rounded-xl sm:rounded-2xl p-4 sm:p-6 shadow-2xl transform rotate-6 sm:rotate-12 hover:rotate-0 transition-transform duration-300 max-w-sm mx-auto lg:max-w-none">
              <div className="text-white mb-3 sm:mb-4">
                <div className="text-xs sm:text-sm text-gray-400 mb-1 sm:mb-2">Cortex AI Live</div>
              ...
              <div className="grid grid-cols-6 sm:grid-cols-8 gap-0.5 sm:gap-1 mb-3 sm:mb-4">
                {pixelColors.map((color, i) => (
                  <div
                    key={i}
                    className={`w-2 h-2 sm:w-3 sm:h-3 ${color}`}
                  />
                ))}
              </div>
              ...
              <div className="flex justify-between items-center text-white text-xs sm:text-sm">
                <div>
                  ...
                  <div className="text-gray-400 text-xs">Status: Processing</div>
                </div>
                <div className="text-right">
                  <div className="mb-1">Documents: 1,247</div>
                  <div className="text-gray-400 text-xs">Speed: 0.3s/doc</div>
                </div>
              </div>
            </div>
        ...
        <div className="mt-12 sm:mt-16 pt-6 sm:pt-8 border-t border-gray-800">
          <div className="flex flex-wrap items-center justify-center gap-4 sm:gap-8 lg:gap-12 opacity-60">
            {["OpenAI", "Microsoft Azure", "AWS", "Google Cloud", "Stripe"].map((company, index) => (
              <div key={index} className="flex items-center space-x-2">
                <div className="w-4 h-4 sm:w-6 sm:h-6 bg-gray-600 rounded"></div>
                <span className="text-gray-400 font-medium text-sm sm:text-base">{company}</span>
              </div>
            ))}
          </div>
        </div>
